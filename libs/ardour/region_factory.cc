...
RegionFactory::map_add (std::shared_ptr<Region> r)
{
	...
	{
		std::lock_guard<std::mutex> lm (region_map_lock);
		region_map.insert (p);
	}
...
RegionFactory::add_to_region_name_maps (std::shared_ptr<Region> region)
{
	...
	std::lock_guard<std::mutex> lm (region_name_maps_mutex);
...
{
	std::unique_lock<std::mutex> lm (region_map_lock);
