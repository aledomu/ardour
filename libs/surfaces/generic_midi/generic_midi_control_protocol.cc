...
GenericMidiControlProtocol::~GenericMidiControlProtocol ()
{
	...
	if (_input_port) {
		...
		std::lock_guard<std::mutex> em (AudioEngine::instance()->process_lock());
...
GenericMidiControlProtocol::drop_all ()
{
	...
	std::lock_guard<std::mutex> lm (pending_lock);
	std::lock_guard<std::mutex> lm2 (controllables_lock);
...
GenericMidiControlProtocol::_send_feedback ()
{
	...
	*/
	...
	std::unique_lock<std::mutex> lm (controllables_lock, std::defer_lock);
	if (!lm.try_lock ()) {
		return;
	}
